...
`journal status` exists for local derived-state introspection only. `watch`, `sync --changed`,
`sync --full`, `journal replay`, and `reconcile` are still Stage 1B V1-C10 targets and are not
implemented yet. `index` and `explore` remain gated future work after Stage 1B and the
official-profile/Obsidian gates.
