...
| V1-C2 | One versioned profile contract | `src/vaultwright/profiles.py` validates schema version 1; `_meta/profile.yml` declares the current `business-operations` template; `docs/PROFILE_SCHEMA.md` documents schema fields, validation rules, migration semantics, and generated-view semantics; package CLI exposes `init --profile business-operations`, `profile list`, `profile show`, `profile validate`, `profile diff`, `profile migrate --plan`, conservative `profile migrate --write`, and `profile views --check/--write`; linter and catalog read profile domains, note types, statuses, required properties, and canonical folders; migration reporting reads profile canonical domains with `_meta/domain-map.yml` as a non-authoritative alias layer | Need remaining core behavior reading profile data consistently before adding more profiles | 1 |
...
| Hard-coded business folders, note types, statuses, and linter assumptions | V1-C2, V1-C3, V1-C7 | Continue extracting into profile contracts before adding more profiles; `lint`, `catalog`, `migration`, and `Documents.base` generation now read profile-defined values |
