...
exists for explicit source/manifest repair queueing. `watch`, `sync --changed`, and `sync --full`
are still Stage 1B V1-C10 targets and are not implemented yet. `index` and `explore` remain gated future work after Stage 1B and the
