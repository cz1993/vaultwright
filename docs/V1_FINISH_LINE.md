...
replay; `sync --full` names the existing full sync recovery path explicitly. `watch` is still a
Stage 1B V1-C10 target and is not implemented yet. `index` and `explore` remain gated future work after Stage 1B and the
