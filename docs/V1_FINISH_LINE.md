...
| --- | --- | --- |
| Copied vault-local scripts | V1-C1 | Refactor into package modules first; leave scripts only as shims during migration; Office sync, repo sync, installed plan/sync/status orchestration, doctor, lint, catalog, conversion, m365, migration, overlap, benchmark, pilot, recovery, and review are the current package-owned examples |
...
`lint`, `catalog`, `conversion`, `m365`, `migration`, `overlap`, `benchmark`, `pilot`, `recovery`, and `review` are implemented against package-owned runtime. Office mirror
planning/sync/status and repo mirror planning/sync/status are package-owned while their vault-local
tools remain compatibility surfaces; vault-local conversion, m365, migration, overlap, benchmark, pilot, recovery, and review-ledger tooling remains as a
