...
| V1-C2 | One versioned profile contract | `src/vaultwright/profiles.py` validates schema version 1, safe domain folders, `folder_plan` paths/domains, safe profile benchmark-task paths, boolean note-type role flags, boolean status-role flags, profile-owned Office mirror placement defaults, and profile-owned generated-mirror status defaults; `_meta/profile.yml` declares the current `business-operations` template with profile-owned machine-owned note type roles plus attention/inactive status roles; `docs/PROFILE_SCHEMA.md` documents schema fields, validation rules, migration semantics, generated-view semantics, `benchmark_tasks`, repo context fields, status roles, `policy_defaults.repo_notes_dir`, `policy_defaults.mirror_mode`, `policy_defaults.mirror_root`, `policy_defaults.mirror_status`, and `policy_defaults.repo_stub_status`; package CLI exposes `init --profile business-operations`, `profile list`, `profile show`, `profile validate`, `profile diff`, `profile migrate --plan`, conservative `profile migrate --write`, and `profile views --check/--write`; profile migration creates missing directories from the profile `folder_plan`; linter and catalog read profile domains, note types, statuses, required properties, and canonical folders; overlap calibration reads profile domain folders when selecting curated notes, profile-defined context fields when counting inbound wikilinks, profile-defined inactive statuses when excluding retired notes, and profile-defined machine-owned note types when excluding generated artifacts; profile-generated Bases read profile-defined attention statuses; benchmark validation and aggregate pilot reporting read profile-declared benchmark task packs; Office sync and lint read profile-owned Office mirror placement defaults while preserving `_meta/mirror-config.yml` as the operator override; Office/repo mirror sync and annotation detection read profile-owned generated-mirror statuses instead of hard-coded `active`/`draft`; GitHub repo mirror sync/lint read the profile default repo-mirror folder, derive repo-mirror domains from profile folder mappings, and derive repo context frontmatter from profile optional properties; annotation migration treats matching profile-defined repo context as generated metadata instead of human annotations; Microsoft 365 handoff, sandbox preflight, recovery, and review-ledger reporting resolve repo mirror folders from the active profile or `tools/repos.yml`; migration reporting reads profile canonical domains with `_meta/domain-map.yml` as a non-authoritative alias layer and ignores profile-defined machine-owned note types during frontmatter cleanup | Need remaining core behavior reading profile data consistently before adding more profiles | 1 |
...
| Agent-readiness benchmark | V1-C8, V1-E10, V1-E11 | Keep benchmark task packs profile-declared and use measured results to decide the Stage 4 Explorer gate |
