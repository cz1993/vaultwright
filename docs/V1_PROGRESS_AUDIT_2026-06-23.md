...
now rejects unsafe domain, note-type, status, frontmatter property, profile artifact path, and
repo-mirror folder default values before they reach runtime paths, views, profile migration, repo
sync, or lint rules. Preserve the example regeneration gates and
