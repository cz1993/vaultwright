...
| Requirement | Status |
| --- | --- |
| V1-C1 package-owned runtime | In progress. Package CLI exists; `plan`, `sync`, `status`, `doctor`, `catalog`, `lint`, `recovery`, and `review` are package-owned; Office mirror planning/sync/status lives in `vaultwright.mirrors.office`; GitHub repo mirror planning/sync/status lives in `vaultwright.mirrors.github_repos`; sync, lint, recovery, review-ledger, and operator-wrapper scripts remain compatibility shims. |
...
likely candidate now that sync orchestration, doctor, lint, catalog, recovery, and review are package-owned. Keep
