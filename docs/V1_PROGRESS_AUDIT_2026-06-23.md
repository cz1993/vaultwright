...
now rejects unsafe domain, note-type, status, frontmatter property, profile artifact path,
repo-mirror folder default, and source-authority/no-real-data policy values before they reach
runtime paths, views, profile migration, repo sync, or lint rules. Preserve the example
