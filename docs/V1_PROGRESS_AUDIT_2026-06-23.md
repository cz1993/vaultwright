...
active Office mirror root. Benchmark task/result
validation and task scaffolding also use the active Office mirror root for generated mirror
evidence, and profile migration now plans the target profile's Office mirror root instead of a
fixed `_mirrors/` directory. Office sync now derives source domains and canonical mirror paths from
active profile domain folders while keeping `_meta/domain-map.yml` as the legacy alias layer. Profile validation
...
unsafe profile artifact paths, invalid context aliases, invalid
...
future profiles. Preserve the example
