...
| Requirement | Status |
| --- | --- |
| V1-C1 package-owned runtime | In progress. Package CLI exists and `catalog` is package-owned, but most runtime still lives in copied template tools. |
...
Move Office mirror planning/sync/status behavior toward package-owned `vaultwright.mirrors` modules
while keeping `template/tools/sync_office_md.py` as a compatibility shim. This advances V1-C1 and
prepares V1-C2/C4 because profile migration and profile-driven domains need package-owned mirror
logic rather than another copied script fork.
