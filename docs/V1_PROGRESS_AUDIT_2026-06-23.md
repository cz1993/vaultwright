...
and make those paths read active profile data consistently. Repo mirror sync/lint, benchmark/pilot
task discovery, and the Microsoft 365, sandbox, recovery, and review-ledger report surfaces now
resolve their profile-controlled paths from profile/config. Preserve the example regeneration gates
and require no-data, lifecycle, recovery, catalog, package-install, benchmark/pilot, and
