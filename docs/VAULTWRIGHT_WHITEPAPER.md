...
| Source-installable CLI | Console entry point supports source-checkout `init`; `catalog` now runs package-owned code while remaining legacy commands delegate to vault-local tools | Good development ergonomics and an initial package-runtime migration path |
