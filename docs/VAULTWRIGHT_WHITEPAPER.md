...
| CLI | Vault-local wrapper and source-installable `vaultwright` entry point; CI now installs the built wheel and verifies packaged init, doctor, plan, benchmark, Markdown/HTML catalog generation/check, conversion, Microsoft 365 handoff readiness, review-ledger summary/check, migration, migration worksheets, pilot, redacted pilot worksheet output, JSON recovery delegation, and recovery worksheet output; tag workflow builds artifacts under read-only permissions, verifies the released wheel, and creates draft prereleases for owner review while refusing to overwrite non-draft releases; doctor reports dependency, manifest, audit, recovery-action counts, git/GitHub backup posture, optional Obsidian config/plugin posture, and `.gitignore` backup guard coverage; benchmark validates task packs, writes private task/result scaffolds, prints private run worksheets, and summarizes optional aggregate result packs; catalog writes the inventory gateway; conversion, m365, migration, pilot, and recovery print read-only operator reports | Better operator ergonomics; first alpha draft release is intended for owner review, not stable distribution |
