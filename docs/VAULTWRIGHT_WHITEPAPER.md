...
| CLI | Vault-local wrapper and source-installable `vaultwright` entry point; CI now installs the built wheel and verifies packaged init, doctor, plan, benchmark, Markdown/HTML catalog generation/check, package-owned benchmark task/result validation, package-owned conversion JSON, package-owned Microsoft 365 handoff JSON, package-owned overlap calibration, package-owned migration JSON, package-owned recovery JSON/worksheet output, review-ledger summary/check, migration worksheets, pilot, and redacted pilot worksheet output; tag workflow builds artifacts under read-only permissions, verifies the released wheel, and creates draft prereleases for owner review while refusing to overwrite non-draft releases; doctor reports dependency, manifest, audit, recovery-action counts, git/GitHub backup posture, optional Obsidian config/plugin posture, and `.gitignore` backup guard coverage; benchmark validates task packs, writes private task/result scaffolds, prints private run worksheets, and summarizes optional aggregate result packs from package-owned code; catalog writes the inventory gateway; conversion now validates private metadata-only quality result packs from package-owned code; pilot prints a read-only operator report while benchmark, conversion, m365, migration, overlap, and recovery use package-owned report code | Better operator ergonomics; first alpha draft release is intended for owner review, not stable distribution |
...
| Source-installable CLI | Console entry point supports source-checkout `init`; installed `plan`, `sync`, `status`, `doctor`, `catalog`, `lint`, `conversion`, `m365`, `migration`, `overlap`, `benchmark`, `recovery`, and `review` now run package-owned code while remaining legacy commands delegate to vault-local tools | Good development ergonomics and a stronger package-runtime migration path |
...
- Source-installable `vaultwright` console entry point via `pyproject.toml`, with installed
  `plan`, `sync`, `status`, `doctor`, `catalog`, `lint`, `conversion`, `m365`, `migration`, `overlap`, `benchmark`, `recovery`, and `review` commands running
