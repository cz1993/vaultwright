...
| Source-installable CLI | Console entry point supports source-checkout `init`; installed `plan`, `sync`, `status`, `doctor`, `catalog`, `lint`, `conversion`, `m365`, `migration`, `overlap`, `benchmark`, `pilot`, `recovery`, and `review` now run package-owned code while remaining legacy commands delegate to vault-local tools | Good development ergonomics and a stronger package-runtime migration path |
...
- Source-installable `vaultwright` console entry point via `pyproject.toml`, with installed
  `plan`, `sync`, `status`, `doctor`, `catalog`, `lint`, `conversion`, `m365`, `migration`, `overlap`, `benchmark`, `pilot`, `recovery`, and `review` commands running
