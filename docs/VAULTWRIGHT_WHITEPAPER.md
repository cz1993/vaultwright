...
| Source-installable CLI | Console entry point supports source-checkout `init`; `catalog`, Office mirror planning/sync/status, and GitHub repo mirror planning/sync/status now run package-owned code while remaining legacy commands delegate to vault-local tools | Good development ergonomics and a stronger package-runtime migration path |
