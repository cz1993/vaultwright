...
| Source-installable CLI | Console entry point supports source-checkout `init`; `catalog` and Office mirror planning/sync/status now run package-owned code while remaining legacy commands delegate to vault-local tools | Good development ergonomics and a stronger package-runtime migration path |
