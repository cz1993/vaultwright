...
| Pilot evidence report | Read-only aggregate report summarizes corpus shape, manifest states, audit counts, conversion priorities, recovery counts, review-ledger counts, benchmark tasks, optional benchmark result scores, and a redacted Markdown worksheet summary without source content, source paths, artifact paths, or reviewer notes; benchmark can now initialize private task and result scaffolds from manifest metadata | Useful for private design-partner worksheets; not external validation by itself |
...
- Private benchmark task and result scaffolds through `vaultwright benchmark --init-tasks` and
