...
def load_tool_module(root: Path, script: str) -> tuple[Any | None, list[str]]:
    ...
    try:
        spec = importlib.util.spec_from_file_location(f"vaultwright_sandbox_{script.replace('.', '_')}", path)
...
def recovery_summary(root: Path) -> tuple[dict[str, Any], list[str], list[str]]:
    ...
    if not module:
        return {"available": False, "items": 0}, [], load_errors
    try:
...
def main() -> int:
    ...
    report, warnings, errors = build_report(ROOT, args.source_root)
