...
def conversion_summary(root: Path) -> tuple[dict[str, Any], list[str], list[str]]:
    try:
        report, warnings, errors = conversion_module.build_report(root, low_risk_per_format=0)
...
def conversion_quality_summary(root: Path) -> tuple[dict[str, Any], list[str], list[str]]:
    ...
    try:
        summary, warnings, errors = conversion_module.validate_quality_results(
            root,
...
def overlap_summary(root: Path) -> tuple[dict[str, Any], list[str], list[str]]:
    try:
        report, warnings = overlap_module.build_report(root, max_pairs=0)
        summary = dict(report.get("summary", {}))
...
def review_summary(root: Path) -> tuple[dict[str, Any], list[str], list[str]]:
    try:
        report, warnings = review_ledger_module.build_report(root)
...
def main(argv: list[str] | None = None, root: Path | None = None) -> int:
    ...
    if args.json:
        ...
        print_human(active_root, report, warnings, errors)
